...
/** Chrome/es-ES suele anteponer ¿ ? ¡ ! al transcribir. */
...
/**
 * Solo devuelve el pedido si la frase empieza con el nombre del bot (palabra clave).
 * Evita activaciones por ruido, eco del altavoz o nombre al final de la frase.
 */
...
/**
 * Interpreta un fragmento de voz: nombre+pedido juntos, solo nombre (armar), o continuación armada.
 */
export function parseWakeSpeechFragment(
  ...
  if (utteranceStartsWithWakeWord(t, wakeWord)) {
    ...
    if (tail.length < minCommandLen) {
      // Solo el nombre: saluda y deja ventana armada para el pedido sin repetir el nombre.
      return {
...
/** ¿Vale la pena procesar este fragmento? Evita TV/radio sin «Karen» al inicio. */
export function shouldAcceptSpeechChunk(
  text: string,
  wakeWord: string | undefined | null,
  armedUntil: number,
): boolean {
  ...
  if (utteranceStartsWithWakeWord(t, wakeWord)) return true;
