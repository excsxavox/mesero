...
function CategoryCard({ cat, panel, reception }: { cat: MenuCategoryPreview; panel?: boolean; reception?: boolean }) {
    ...
    >
      <div className="mb-3 flex shrink-0 items-center justify-between gap-2">
        <h2 className="text-xs font-semibold uppercase tracking-wide text-mesero-text-muted">
          {reception ? COPY.offersTitle : "Categorías"}
        </h2>
        {onViewCatalog ? (
          <button
            type="button"
            ...
            className="touch-manipulation text-xs font-medium text-mesero-accent hover:text-teal-200 disabled:opacity-50"
          >
            {reception ? COPY.offersViewAll : "Ver catálogo →"}
          </button>
        ) : null}
      </div>
      {categories.length > 0 ? (
        ...
        >
          <div className={panel ? "grid grid-cols-2 gap-2 sm:grid-cols-3" : "flex gap-2"}>
            {categories.map((cat) => (
              ...
              >
                <CategoryCard cat={cat} panel={panel} reception={reception} />
              </button>
            ))}
          </div>
        </div>
      ) : (
        <p className="text-sm text-mesero-text-muted/80">
          {reception
            ? "Aún no hay tours ni productos en el catálogo. Configúralos en AIBox o el menú del servidor."
