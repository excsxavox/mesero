...
function QrImage({ dataUrl, error, size }: { dataUrl: string | null; error: string | null; size: number }) {
  return (
    <div className="rounded-xl bg-white p-2 shadow-md shadow-black/30">
      {dataUrl ? (
        <img src={dataUrl} alt="Código QR de información" width={size} height={size} className="block" />
      ) : error ? (
        <div
...
function MenuQrModal({
  ...
  const title = variant === "reception" ? COPY.infoQrTitle : "Menú";
  const hint = variant === "reception" ? COPY.infoQrHint : "Escanea para ver la carta en tu móvil";
    ...
    >
      ...
      >
        <header className="mb-4 flex items-start justify-between gap-3">
          <div>
            <h2 id="menu-qr-modal-title" className="text-sm font-semibold uppercase tracking-wide text-teal-200/90">
              {title}
            </h2>
...
export function MenuQrCard({ compact = false, variant = "reception" }: Props) {
  const title = variant === "reception" ? COPY.infoQrTitle : "Menú";
  const hint = variant === "reception" ? COPY.infoQrHint : "Escanea para ver la carta en tu móvil";
  const compactHint = variant === "reception" ? COPY.infoQrCompact : "Escanear carta en el móvil";
    ...
    return (
      <>
        <section className="rounded-2xl border border-mesero-line/15 bg-mesero-panel/90 px-3 py-2.5 ring-1 ring-mesero-line/10">
          <h2 className="text-[11px] font-semibold uppercase tracking-wide text-mesero-text-muted">{title}</h2>
          <p className="mt-0.5 text-[10px] leading-snug text-mesero-text-muted/80">{compactHint}</p>
          <button
            type="button"
            onClick={openModal}
        ...
        <MenuQrModal open={modalOpen} onClose={closeModal} publicUrl={publicUrl} variant={variant} />
      </>
    );
  }
  ...
  return (
    <section className="flex flex-col items-center rounded-2xl border border-mesero-line/15 bg-mesero-panel/90 p-4 ring-1 ring-mesero-line/10">
      <h2 className="text-xs font-semibold uppercase tracking-wide text-mesero-text-muted">{title}</h2>
