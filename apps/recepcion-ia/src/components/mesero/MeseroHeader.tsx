...
import type { RecepcionTheme } from "../../lib/recepcionTheme";
...
function IconReception() {
  return (
    <span className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-mesero-accent to-teal-600 text-lg shadow-lg shadow-mesero-deep/40">
      🛎️
    </span>
  );
}
...
export function RecepcionHeader({
  restaurantName,
  tableLabel,
  lockArmed,
  onPadlockClick,
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-mesero-text-muted">
            {tableLabel ? (
              ...
              "Modo recepción · candado activo"
            ) : (
              "Hotel y complejo privado · recepción con IA"
            )}
          </p>
        </div>
      </div>
