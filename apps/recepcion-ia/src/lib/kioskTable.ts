...
import { saveKioskTable } from "./recepcionSessionStorage";

/** Lee ?puesto=2, ?mesa=4 o ?table=4 de la URL. */
export function parseTableFromSearchParams(params: URLSearchParams): number | null {
  const raw = params.get("puesto") ?? params.get("mesa") ?? params.get("table") ?? params.get("mesaId");
