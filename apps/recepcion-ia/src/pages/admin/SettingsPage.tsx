...
import { loadRecepcionSession, saveRecepcionSession } from "../../lib/recepcionSessionStorage";
...
export function SettingsPage() {
  ...
  const assignKioskTable = (table: number) => {
    ...
    const session = loadRecepcionSession();
    saveRecepcionSession({ ...session, selectedTable: clamped });
    setStatus(
      `Mostrador ${clamped} asignado a esta pantalla. Otros equipos pueden usar otro número (Admin en cada pantalla o ?puesto=N en la URL).`,
    );
  };
      ...
      .then((s) => {
        const kt =
          typeof s.kioskTable === "number" && Number.isFinite(s.kioskTable)
            ...
            : null;
        const local = loadRecepcionSession().selectedTable;
  ...
  const clearExitPassword = async () => {
    if (
      !window.confirm(
        "¿Quitar la contraseña del candado? La recepción no podrá bloquear administración hasta que definas una nueva.",
      )
    ) {
      return;
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm leading-relaxed text-zinc-500">
          Nombre del hotel o complejo, mostradores de recepción, palabra de activación e instrucciones del recepcionista. El idioma se detecta
      ...
      <section className="max-w-2xl space-y-4 rounded-xl border border-zinc-800 bg-zinc-950/40 p-4 ring-1 ring-zinc-800/80">
        <h2 className="text-sm font-semibold text-zinc-200">Recepcionista virtual</h2>

        <label className={labelClass}>
          Nombre del hotel o complejo
          <input
        ...
        <label className={labelClass}>
          ...
          <span className="mt-1.5 block text-xs leading-relaxed text-zinc-500">
            El visitante la dice para activar el micrófono (p. ej. «{assistantPreview}, tengo cita a las 10»). Solo
            letras y números, 2–24 caracteres. En pantalla se muestra como{" "}
      ...
      <section className="max-w-2xl space-y-4 rounded-xl border border-zinc-800 bg-zinc-950/40 p-4 ring-1 ring-zinc-800/80">
        <h2 className="text-sm font-semibold text-zinc-200">Mostradores de recepción</h2>
        <p className="text-sm leading-relaxed text-zinc-500">
          Cuántas pantallas hay en lobby, acceso principal o portería. Asigna el mostrador de este dispositivo.
        </p>
        ...
        <label className={labelClass}>
          ...
          />
          <span className="mt-1.5 block text-xs leading-relaxed text-zinc-500">
            Mostradores numerados del 1 al valor indicado ({MIN_TABLE_COUNT}–{MAX_TABLE_COUNT}). Guarda los cambios con el
        ...
        <p className="text-xs leading-relaxed text-zinc-500">
          Cada pantalla guarda su mostrador en este navegador. Para varios equipos en lobby, configura cada uno aquí o abre{" "}
          <code className="rounded bg-zinc-950 px-1 text-amber-200/90">?puesto=2</code> (ej. mostrador 2).
        </p>
      ...
      <section className="max-w-2xl space-y-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-4 ring-1 ring-zinc-800/80">
        <h2 className="text-sm font-semibold text-zinc-200">Candado de recepción</h2>
        <p className="text-sm leading-relaxed text-zinc-500">
          Contraseña para quitar el candado o entrar en Administración desde la pantalla principal de recepción. No se
