...
import { ReceptionProfileCard } from "../components/mesero/ReceptionProfileCard";
import { VoiceOrb } from "../components/mesero/VoiceOrb";
import { RecepcionHeader } from "../components/mesero/MeseroHeader";
...
import { useRecepcionTheme } from "../context/RecepcionThemeContext";
import { DEFAULT_ORGANIZATION_NAME } from "../lib/receptionCopy";
import { saveRecepcionSession } from "../lib/recepcionSessionStorage";
...
export function ChatPage() {
  ...
  const { theme, toggleTheme } = useRecepcionTheme();
  ...
  const bumpTouchDelta = useCallback(
  ...
  const onPadlockClick = () => {
    if (!lockArmed) {
      ...
      setToast(
        "Candado activo: pantalla bloqueada en recepción. Usa la contraseña de administrador para desbloquear.",
      );
      setTimeout(() => setToast(null), 5000);
      return;
    }
  ...
  return (
    ...
    >
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-amber-500/30 bg-zinc-900 p-6 shadow-2xl ring-1 ring-amber-500/20">
            ...
            <p id="mandatory-password-desc" className="mt-2 text-sm leading-relaxed text-zinc-400">
              Aún no hay contraseña de administrador para el candado de recepción. Debes crearla aquí para continuar: la
          ...
          <div className="w-full max-w-sm rounded-2xl border border-zinc-700 bg-zinc-950 p-5 shadow-xl ring-1 ring-zinc-800">
            ...
            <p className="mt-1 text-sm text-zinc-500">
              {adminGate.kind === "disarm"
                ? "Introduce la contraseña que guardaste en Administración → Configuración (Candado de recepción)."
      ...
      <RecepcionHeader
        restaurantName={profileCompanyName || settings?.restaurantName?.trim() || DEFAULT_ORGANIZATION_NAME}
      ...
      {openAiOnServer === false ? (
        ...
        >
          ...
          <code className="rounded bg-zinc-900 px-1 py-0.5 text-zinc-300">.env</code> de la raíz del monorepo sin
          espacios alrededor del valor, <strong className="font-medium text-amber-200">guarda el archivo en disco</strong>{" "}
          (Ctrl+S) y reinicia con <code className="text-zinc-300">npm run dev</code>.
        </div>
      ) : null}
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain">
              <div className="grid grid-cols-1 gap-4 pb-2 md:grid-cols-[minmax(0,1fr)_minmax(17rem,34%)] md:items-start md:[grid-template-areas:'profile_side'_'voice_side'_'offers_side'_'hints_side']">
            <div className="min-w-0 md:[grid-area:profile]">
              <ReceptionProfileCard
            ...
            <div className="flex min-w-0 flex-col gap-3 md:[grid-area:side] md:row-span-4 md:self-start">
              ...
              <MenuQrCard variant="reception" compact={hasActiveBooking} />
              <OrderSummaryCard
                variant="reception"
                menu={menu}
                corpus={orderDraftCorpus}
                ...
                onClearOrder={
                  !needsMandatoryPasswordSetup && (hasActiveBooking || confirmed.length > 0)
                    ? handleClearOrder
                    : undefined
                }
            ...
            <div className="min-w-0 md:[grid-area:offers]">
              <MenuCategoryStrip
                variant="reception"
                layout="panel"
        ...
        <footer className="relative z-10 shrink-0 border-t border-mesero-line/15 bg-mesero-deep/95 pt-3 backdrop-blur-sm">
          <div className="flex gap-2 rounded-2xl border border-mesero-line/10 bg-mesero-deep/20 p-2">
            <input
              ...
              placeholder={
                needsMandatoryPasswordSetup
                  ? "Define la contraseña en el cuadro emergente para usar la recepción"
            ...
            <button
              ...
              disabled={busy || needsMandatoryPasswordSetup}
              onClick={send}
              className="touch-manipulation flex min-h-12 min-w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-r from-mesero-accent to-teal-600 px-4 text-white hover:from-teal-400 hover:to-teal-500 disabled:opacity-50"
            >
              {busy ? (
